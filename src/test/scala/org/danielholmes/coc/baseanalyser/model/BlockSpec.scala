...
class BlockSpec extends FlatSpec with Matchers {
  ...
  it should "return empty internal coords when 1x1" in {
    Block(Tile(0, 0), TileSize(1)).internalCoordinates should be (Set.empty)
  }
  ...
  it should "return true intersect for overlapping items" in {
    Block(Tile.Origin, TileSize(2)).intersects(Block(Tile(1, 1), TileSize(2))) should be (true)
  }
  ...
  it should "return false any intersect for no items" in {
    Block.anyIntersect(Set(Block(Tile.Origin, TileSize(2)))) should be (false)
  }
  ...
  it should "return false any intersect for edge touching items" in {
    Block.anyIntersect(Set(Block(Tile.Origin, TileSize(2)), Block(Tile(0, 2), TileSize(2)))) should be (false)
  }

  it should "return true any intersect for overlapping items" in {
    Block.anyIntersect(Set(Block(Tile.Origin, TileSize(2)), Block(Tile(1, 1), TileSize(2)))) should be (true)
  }

  it should "return correct all tiles" in {
    Block(Tile.Origin, TileSize(2)).tiles should contain theSameElementsAs Set(Tile.Origin, Tile(0, 1), Tile(1, 0), Tile(1, 1))
  }
