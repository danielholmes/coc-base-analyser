...
class HighHPUnderAirDefRuleSpec extends FlatSpec with Matchers {
  ...
  it should "return fail for base with air def and storage outside" in {
    ...
    rule.analyse(Village(Set(AirDefense(1, TileCoordinate.Middle), storage))) should be (HighHPUnderAirDefResult(Set(storage)))
  }
  ...
  it should "return fail for base with air def cutting storage" in {
    ...
    rule.analyse(Village(Set(AirDefense(1, TileCoordinate.Origin), storageOutside, storageInside))) should be (HighHPUnderAirDefResult(Set(storageOutside)))
  }
