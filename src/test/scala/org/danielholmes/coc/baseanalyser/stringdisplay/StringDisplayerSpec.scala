...
class StringDisplayerSpec extends FlatSpec with Matchers {
  ...
  it should "display origin town hall" in {
    val result = displayer.build(Village(Set(TownHall(1, TileCoordinate(0, 0)))))
    for (row <- 1 to 4) {
      for (col <- 1 to 4) {
  ...
  it should "display end of earth archer tower" in {
    val result = displayer.build(Village(Set(ArcherTower(1, TileCoordinate(41, 41)))))
    for (row <- 42 to 44) {
      for (col <- 42 to 44) {
  ...
  it should "display end of map cc radius without exception" in {
    displayer.build(Village(Set(ClanCastle(1, TileCoordinate(41, 41)))))
